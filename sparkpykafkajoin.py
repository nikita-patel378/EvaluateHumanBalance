...
spark = SparkSession.builder.appName("customer-risk-app").getOrCreate()
...
redisServerStreamingDF.withColumn("value", from_json("value", redisInfoSchema))\
    .select("value.*")\
...
zSetDecodedEntriesStreamingDF\
    .withColumn("customer",from_json("encodedCustomer", customerJSONSchema))\
    .select(col("customer.*"))\
...
kafkaStreamingDF.withColumn("value", from_json("value", customerRiskJSONSchema))\
    .select("value.*")\
...
customerRiskAndBirthYearStreamingDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr( """
   customer=email
"""))
...
customerRiskAndBirthYearStreamingDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value")\
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
